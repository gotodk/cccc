...
public class UserLock
{
    ...
    public static string LockBegin_ByUser(string khbh)
    {
        ...
        input["@KHBH"] = khbh;
        Hashtable return_ht = I_DBL.RunProc_CMD("LOCKmessage_CheckLock", "锁", input);
        string re = "0";
            ...
            if (dsGXlist != null && dsGXlist.Tables[0].Rows.Count > 0)
            {
                    ...
                    if (dr[0].ToString().Trim().Equals("用户早已被锁定"))
                {
                    for (int i = 0; i < 10; i++)
                    {
                        ...
                        Hashtable return_htlockde = I_DBL.RunProc_CMD("LOCKmessage_CheckLock", "锁", input);
                    ...
                    //强制解锁
                    Hashtable return_htunlock = I_DBL.RunProc_CMD("LOCKmessage_UnLock", "锁", input);
    ...
    public static string LockEnd_ByUser(string khbh)
    {
        ...
        Hashtable return_ht = I_DBL.RunProc_CMD("LOCKmessage_UnLock", "锁", input);

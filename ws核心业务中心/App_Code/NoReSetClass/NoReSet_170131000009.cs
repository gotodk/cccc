...
public class NoReSet_170131000009
{
    ...
    public DataSet NRS_EDIT(DataTable parameter_forUI)
    {
        ...
        if (Convert.ToInt32(ht_forUI["dqyqmsl"]) + Convert.ToInt32(ht_forUI["xzsl"]) > Convert.ToInt32(ht_forUI["ss_yqmsx"]))
        {

            dsreturn.Tables["返回值单条"].Rows[0]["执行结果"] = "err";
            dsreturn.Tables["返回值单条"].Rows[0]["提示文本"] = "申请失败！已申请数量+本次申请数量超过了可申请上限。";
            return dsreturn;
        }
        ...
        param.Add("@xzsl", ht_forUI["xzsl"].ToString());
        
      
        alsql.Add("  update AAA_SJS set beok=1,userid=@UAid,sqsj=getdate() where SN in (select top " + ht_forUI["xzsl"].ToString() + " SN from AAA_SJS where beok=0 and joinok=0 order by newid()) ");
        ...
        if ((bool)(return_ht["return_float"]))
        {
            ...
            dsreturn.Tables["返回值单条"].Rows[0]["提示文本"] = "新的邀请码申请成功！";
        }
        else
        {
    ...
    public DataSet NRS_EDIT_INFO(DataTable parameter_forUI)
    {
        ...
        return_ht = I_DBL.RunParam_SQL("select  top 1 * from View_J_my_yqm_info where UAid=@UAid", "数据记录", param);
